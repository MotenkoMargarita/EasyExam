...
function addImage(questionImages) {
        ...
        fetch('/upload/image', {method: "POST", body: formData})
            ...
            .then(function (data) {
                if (data !== 'Проблемы при загрузке файла' && data !== 'Выбирите другой формат файла') {
                    ...
                    img.id = "questionImage" + String(counter);
                    ...
                    hiddenInput.setAttribute("name", "questionImageInput" + String(counter));
                    hiddenInput.id = "questionImageInput" + String(counter);
...
function deleteImage(imageNumber) {
    ...
    let hiddenInput = document.getElementById("questionImageInput" + imageNumber);
    hiddenInput.remove();

    let img = document.getElementById("questionImage" + imageNumber);
    img.remove();
}