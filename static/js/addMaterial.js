...
function addImage(materialImages) {
        ...
        fetch('/upload/image', {method: "POST", body: formData})
            ...
            .then(function (data) {
                if (data !== 'Проблемы при загрузке файла' && data !== 'Выбирите другой формат файла') {
                    ...
                    img.id = "materialImage" + String(counter);
                    img.style.maxWidth = "100%";
                    img.style.maxHeight = "300px";
                    img.style.minHeight = "200px";
                    let images = document.getElementById(materialImages);
                    images.appendChild(img);

                    let hiddenInput = document.createElement("input");
                    hiddenInput.setAttribute("name", "materialImageInput" + String(counter));
                    hiddenInput.id = "materialImageInput" + String(counter);
...
function deleteImage(imageNumber) {
    ...
    let hiddenInput = document.getElementById("materialImageInput" + imageNumber);
    hiddenInput.remove();

    let img = document.getElementById("materialImage" + imageNumber);
    img.remove();
}